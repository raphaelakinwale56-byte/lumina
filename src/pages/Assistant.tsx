...
import { Sparkles, Send, Bot, User, BookOpen, Loader2, RefreshCw } from 'lucide-react';
...
export default function Assistant() {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'assistant', content: "Greetings, bibliophile. I am Lumina, your literary guide. What kind of stories are you searching for today? Tell me about your favorite genres, or ask for a recommendation based on a mood." }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: "I apologize, but my literary senses are a bit clouded at the moment. Could you try asking again?" }]);
    } finally {
      setIsTyping(false);
    }
  };
      ...
      <div className="w-full max-w-4xl flex-1 flex flex-col glass-card rounded-[2rem] overflow-hidden shadow-2xl">
        ...
        <div className="p-6 border-b border-brand-200 flex items-center justify-between bg-white/50">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-xl font-serif font-bold text-brand-900">Lumina Assistant</h1>
              <p className="text-xs text-brand-500 font-medium">Your personal literary guide</p>
            </div>
          </div>
          <button 
            onClick={() => setMessages([{ role: 'assistant', content: "Greetings, bibliophile. I am Lumina, your literary guide. What kind of stories are you searching for today?" }])}
            ...
            >
              <div className={`flex max-w-[85%] space-x-4 ${msg.role === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                <div className={`w-10 h-10 rounded-xl flex-shrink-0 flex items-center justify-center shadow-sm ${
                  msg.role === 'user' ? 'bg-brand-200 text-brand-700' : 'bg-brand-800 text-white'
                }`}>
            ...
            <div className="flex justify-start">
              <div className="flex space-x-4">
                ...
                <div className="bg-white p-5 rounded-2xl rounded-tl-none border border-brand-100 flex items-center space-x-2">
                  ...
                  <span className="text-sm text-brand-500 font-medium italic">Lumina is thinking...</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="p-6 bg-white/50 border-t border-brand-200">
          <div className="relative flex items-center">
            ...
            <button
              ...
              className="absolute right-2 p-3 bg-brand-800 text-white rounded-xl hover:bg-purple-600 transition-all disabled:opacity-50 disabled:scale-100 active:scale-95 shadow-lg"
            >
              <Send className="w-5 h-5" />
            </button>
          </div>
          <p className="mt-3 text-center text-[10px] text-brand-400 font-medium uppercase tracking-widest">
